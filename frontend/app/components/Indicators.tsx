...
export default function Indicators({ symbol }: { symbol: string }) {
  ...
  if (!data) return <div className="bg-[var(--color-bg-panel)] border border-[var(--color-border)] p-6 rounded-lg h-full flex items-center justify-center text-[color:var(--color-text-secondary)] font-mono">Loading Indicators...</div>;
  ...
  return (
    <div className="bg-[var(--color-bg-panel)] border border-[var(--color-border)] rounded-lg shadow-lg font-mono flex flex-col hover:border-gray-500 transition-all">
      <div className="p-4 border-b border-[var(--color-border)] flex justify-between items-center">
        <span className="text-[color:var(--color-text-secondary)] text-sm font-bold font-sans">INDICATORS</span>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-1">
            ...
            <span className="text-[color:var(--color-text-secondary)] text-xs ml-1">{data.signal_strength}/10</span>
          </div>
      ...
      <div className="grid grid-cols-2 divide-x divide-y divide-[#222222]">
        ...
        <div className="p-4 flex flex-col gap-2">
          <div className="text-[color:var(--color-text-secondary)] text-xs">RSI (14)</div>
          ...
          <div className="text-2xl font-bold mt-1 text-[color:var(--color-text-primary)]">{data.rsi?.toFixed(2)}</div>
          ...
          <div className="flex justify-between items-center text-xs">
            ...
            <span className={`${data.rsi_zone === 'OVERSOLD' || data.rsi_zone === 'OVERBOUGHT' ? 'text-[#ff4466]' : 'text-[color:var(--color-text-secondary)]'}`}>{data.rsi_zone}</span>
          </div>
        </div>

        {/* MACD */}
        <div className="p-4 flex flex-col gap-2">
          <div className="text-[color:var(--color-text-secondary)] text-xs">MACD</div>
          ...
          <div className="text-xs text-[color:var(--color-text-secondary)]">Signal: {data.macd?.toFixed(2)}</div>
        </div>

        {/* EMA 20 */}
        <div className="p-4 flex flex-col gap-1">
          <div className="text-[color:var(--color-text-secondary)] text-xs">EMA 20</div>
          <div className="text-lg font-bold text-[color:var(--color-text-primary)]">${data.ema20?.toLocaleString(undefined, {maximumFractionDigits: 2})}</div>
          ...
          <div className="text-xs text-[color:var(--color-text-secondary)]">{data.price_vs_ema20 > 0 ? 'price above' : 'price below'}</div>
        </div>

        {/* EMA 50 */}
        <div className="p-4 flex flex-col gap-1">
          <div className="text-[color:var(--color-text-secondary)] text-xs">EMA 50</div>
          <div className="text-lg font-bold text-[color:var(--color-text-primary)]">${data.ema50?.toLocaleString(undefined, {maximumFractionDigits: 2})}</div>
          <div className="text-sm">
          ...
          <div className="text-xs text-[color:var(--color-text-secondary)]">{data.price_vs_ema50 > 0 ? 'price above' : 'price below'}</div>
        </div>
      </div>
    </div>
  );
