...
export default function ManualTradeHistory({ symbol = 'BTCUSDT' }: { symbol?: string }) {
  const [trades, setTrades] = useState<Trade[]>([]);

  useEffect(() => {
    const fetchTrades = async () => {
      try {
        const res = await fetch(`${API_URL}/api/manual-trades?symbol=${symbol}`);
  ...
  return (
    <div className="bg-[var(--color-bg-panel)] border border-[var(--color-border)] p-6 rounded-lg shadow-lg flex flex-col min-h-[280px]">
      <div className="text-[color:var(--color-text-secondary)] text-sm font-medium uppercase mb-4">
        {symbol.replace('USDT', '')} Manual Trade History
      </div>
      ...
      <div className="overflow-x-auto flex-grow">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-[var(--color-border)]/50">
            {trades.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-8 text-center text-[color:var(--color-text-secondary)]">No manual trades yet</td>
              </tr>
            ) : (
              trades.map((trade, i) => (
                <tr key={i} className="hover:bg-[var(--color-bg-hover)]/20 transition-colors" title={trade.reason}>
                  ...
                  <td className="py-3 text-[color:var(--color-text-primary)] text-right">${(trade.quote_amount || 0).toFixed(2)}</td>
